...
def validation(sess, feeds, fetches, dataset: ImgDataset, converter, result_dir, name,
    ...
    for batch in range(dataset.num_batches):
        ...
        log_prob, batch_predicts, edit_distance, batch_edit_distances, outputs_pos, decoded = sess.run(fetches, feed)
        ...
        print('indexee:', outputs_pos)
        print('indexxx:',np.argmax(outputs_pos, axis=1))
        print('batch_positionss:', positions)
